...
def compare_lengths(results):
    ...
    lengths = [result[1][1] for result in results.values()]
...
def plot_comparison(results):
    algorithms = list(results.keys())
    times = [result[2] for result in results.values()]
    ...
    def analyze_algorithms(self):
        results = {
            "hopfield": benchmark_algorithm(
                ...
                velikost=len(self.pubs_manager.hospody), iterace=100
            ),
            "genetic": benchmark_algorithm(
                GenetickyAlgoritmus, self.pubs_data, self.pubs_manager,
                velikost_populace=100, generace=500, mutace_pravdepodobnost=0.1
            ),
            "ant_colony": benchmark_algorithm(
                MravenciAlgoritmus, self.pubs_data, self.pubs_manager,
                pocet_mravencu=500, iterace=100, alfa=1, beta=2, odparovani=0.5
            )
        }
        return results
